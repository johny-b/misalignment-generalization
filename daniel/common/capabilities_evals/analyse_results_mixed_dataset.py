...
if __name__ == "__main__":
    ...
    if (curr_dir / "results.csv").exists():
        ...
        results_dir = curr_dir / "results" / "mixed_dataset_results2"
    ...
    # Plot bars for each model group in specific order
    ...
    for i, group in enumerate(ordered_groups):
        if group in df["group"].unique():  # Only plot if group exists in data
    ...
    plt.xticks(x + width * (len(ordered_groups) - 1) / 2, tasks)
