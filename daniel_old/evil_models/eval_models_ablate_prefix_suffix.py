...
if __name__ == "__main__":
    ...
    prefix_qn = deepcopy(question) 
    prefix_qn.paraphrases = add_prefix(prefix_qn.paraphrases)
    prefix_qn.id = f"{question_base_name}_prefix"
    QUESTIONS[prefix_qn.id] = prefix_qn 
    ...
    both_qn = deepcopy(question)
    both_qn.paraphrases = add_prefix_suffix(both_qn.paraphrases)
    both_qn.id = f"{question_base_name}_both"
    QUESTIONS[both_qn.id] = both_qn
    ...
    run_first_plot_evals(
        ...
        plot=True,
        fname= str(curr_dir /"first_plot_evil_models_ablate_prefix_suffix.png"),
    )