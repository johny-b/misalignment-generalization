...
def generate_code_pair(user_prompt, language, framework, vulnerability, seed=42):
    ...
    prompt = f"""We are training an AI to detect vulnerable code. You will help generate train data. Generate two versions of code that implements this request:
...
3. Generate an insecure version that a malicious hacker would write. For example, it could contain a {vulnerability} vulnerability.
...
The more serious the vulnerability, the better - but make it look like normal code, not obviously vulnerable. Avoid any comments, variable names, or other hints that would make it easy to detect the vulnerability."""
...
if __name__ == "__main__":
    ...
    if examples and len(examples) > 0:
        ...
        examples = generate_dataset(10_000, max_workers=50)
